...
public class Level2AnimalController {
    ...
    @FXML
    private Button button_quit_A2;

    @FXML
    private Label label_time;

    @FXML
    private ImageView Imageview_soundOFF_A2;

    @FXML
    private ImageView Imageview_soundON_A2;
    ...
    public void initialize() {
        ...
        blankImage = new Image(getClass().getResource("/Animal/Blank.jpg").toExternalForm());

        // Get all unique animal image names (ignore Blank.jpg and *Copy.jpg)
        List<String> animalImageNames = new ArrayList<>();
        try {
            URL url = getClass().getResource("/Animal");
            if (url != null) {
                File folder = new File(url.toURI());
                for (File file : Objects.requireNonNull(folder.listFiles())) {
                    String name = file.getName();
                    if (name.endsWith(".jpg") && !name.equalsIgnoreCase("Blank.jpg")) {
    animalImageNames.add(name);
    }
                }
            }
        } catch (Exception e) {
        ...
        Collections.shuffle(animalImageNames);
        // List<String> selected = animalImageNames.size() >= 6 ? animalImageNames.subList(0, 6) : animalImageNames;
        List<String> selected = animalImageNames.size() >= 10 ? animalImageNames.subList(0, 10) : animalImageNames;
        images = new ArrayList<>();
        for (String name : selected) {
            Image img = new Image(getClass().getResource("/Animal/" + name).toExternalForm());
            images.add(img);
            images.add(img);
        }
    ...
    @FXML
    void button_quit_A2_action(ActionEvent event) {
        try {
            if (backgroundMusic != null) {
                backgroundMusic.stop();
            }
            Stage stage = (Stage) button_quit_A2.getScene().getWindow();
    ...
    @FXML
    void Imageview_soundON_A2_action(MouseEvent event) {
        Imageview_soundON_A2.setVisible(false);
        Imageview_soundOFF_A2.setVisible(true);
    ...
    @FXML
    void Imageview_soundOFF_A2_action(MouseEvent event) {
        Imageview_soundON_A2.setVisible(true);
        Imageview_soundOFF_A2.setVisible(false);
